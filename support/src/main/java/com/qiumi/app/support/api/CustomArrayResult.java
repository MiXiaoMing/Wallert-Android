...
public abstract class CustomArrayResult<T extends HttpArrayResult> implements Observer<T> {
    ...
    public void onNext(T result) {
        int status = Integer.valueOf(result.status, 0);
        if (status <= 0) {
            Logger.getLogger().e("网络请求错误：，status：" + result.status + "  message：" + result.message);
